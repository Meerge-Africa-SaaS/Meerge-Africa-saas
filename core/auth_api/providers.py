...
@router.post('/social/facebook/authenticate', response={200: AuthResponseSchema, 404: NotFoundSchema})
@transaction.atomic
def facebook_authenticate(request, data: SocialAuthSchema):
    access_token = data.access_token
    actor_type = data.actor_type
    provider = "facebook"
    
    user_info = get_facebook_details(access_token)
    if not user_info:
        return 404, {"message": "Invalid token"}
